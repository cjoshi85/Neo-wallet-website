...
export const loadAddresses = () => (dispatch: DispatchType) => {
  localStorage.getItem(ADDRESS_BOOK_STORAGE_KEY, (error, data) => {
    if (error) {
...
export const saveAddress = (address: string, name: string) => (dispatch: DispatchType) => {
  ...
  return localStorage.getItem(ADDRESS_BOOK_STORAGE_KEY, (error, data) => {
    if (error) {
      dispatch(showErrorNotification({ message: 'Error loading address book.' }))
    } else if (data[name]) {
      ...
      localStorage.setItem(ADDRESS_BOOK_STORAGE_KEY, addresses, (error) => {
        if (error) {
