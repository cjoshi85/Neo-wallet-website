...
export const upgradeUserWalletNEP6 = (): Promise<*> => {
  return new Promise((resolve, reject) => {
    localStorage.getItem('userWallet', (readNEP6Error, data) => {
      if (readNEP6Error) {
        reject(readNEP6Error)
      }

      if (isEmpty(data)) {
        localStorage.getItem('keys', (readLegacyError, keyData) => {
          if (readLegacyError) {
          ...
          if (isEmpty(keyData)) {
            ...
            localStorage.setItem('userWallet', wallet, (saveError) => {
              if (saveError) {
                reject(saveError)
              } else {
                resolve()
...
export const recoverWallet = (wallet: Object): Promise<*> => {
  return new Promise((resolve, reject) => {
    debugger
    localStorage.getItem('userWallet', (readError, data) => {
      if (readError) {
        reject(readError)
      }
      ...
      localStorage.setItem('userWallet', data, (saveError) => {
        if (saveError) {
          reject(saveError)
        } else {
          resolve(data)
